...
Capstone submission documents live in [`deliverables/`](deliverables/), with
[`SUBMISSION-INDEX.md`](deliverables/SUBMISSION-INDEX.md) mapping each to its checklist item.
...
browsable `index.html`; [`docs/evidence/`](docs/evidence/) holds the raw test output; and
`scripts/docs/` regenerates all of it.

### Two corrections worth knowing

**Measured latency.** The end-to-end conversation pipeline measures **27,457 ms p50** on the
live instance — Whisper 4,004 ms, GPT-4o 3,471 ms, ElevenLabs **19,618 ms (71%)**. The
"3–6 second" figure in the Phase 3 document traces to the admin console's *demonstration
fixtures*, not a measurement. NFR1 required five seconds; it is not met. See Section 3.5 of
the report.

**Test count.** 141 automated tests pass — 66 backend across 10 Jest suites, 75 client across
9 Flutter files. `TESTING.md` still says 62/137 and is stale.

### A note on the repository name

This repository is called `rn-final-manish` and contains **no React Native**. The mobile
application is Flutter/Dart. The name is a leftover from an early decision that was reversed
