...
-- 8. VERIFY
-- ================================================
-- The console admin no longer needs a users row: /api/admin/login authorises
-- from ADMIN_EMAIL/ADMIN_PASSWORD and mints a synthetic principal, so nothing
-- has to be promoted here. This just confirms the objects above exist.
SELECT 'is_admin column' AS object,
       to_regclass('public.users') IS NOT NULL
         AND EXISTS (SELECT 1 FROM information_schema.columns
                     WHERE table_name = 'users' AND column_name = 'is_admin') AS ok
UNION ALL SELECT 'conversation_logs', to_regclass('public.conversation_logs') IS NOT NULL
UNION ALL SELECT 'webhook_events',    to_regclass('public.webhook_events')    IS NOT NULL
UNION ALL SELECT 'feature_flags',     to_regclass('public.feature_flags')     IS NOT NULL
